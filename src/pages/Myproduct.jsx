...
function Editproduct() {
    ...
    const handleDelete = (id) => {
        Swal.fire('Delete product', 'Are you sure to delete this product', 'question')
            .then((result) => {
                            ...
                            <div className='flex flex-col gap-4 p-4'>
                                {
                                    myProducts.length < 1 ? (
                                        ...
                                        myProducts.map((product) => (
                                            <div className='shadow rounded-lg overflow-hidden bg-white flex items-center' key={product.productID}>
                                                <img src={product.image} alt="" className='w-48 cursor-pointer'/>
                                                ...
                                                <div className='text-center gap-4 flex flex-col px-14'>
                                                    <button className='shadow-md rounded py-2 px-10 font-bold bg-red-600 text-white' id={`edit-product-${''}`} onClick={() => navigate(`/edit-product/${product.productID}`)}>Edit</button>
                                                    <button className='shadow-md rounded py-2 px-10 font-bold text-red-600' id={`del-product-${''}`} onClick={() => handleDelete(product.productID)}>Delete</button>
                                                </div>
                                            </div>
                                        ))    
                                    )
