...
function ListSubmissionUmkm() {
  ...
  const handleStats = (id, status) => {
    Swal.fire(
      "Update",
      `Are you sure to ${status} this event`,
      "question"
    ).then((res) => {
      if (res.isConfirmed) {
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <a
            href={`https://api.WhatsApp.com/send?phone=${params.value}`}
      ...
      renderCell: (params) => {
        ...
        return (
          <a
            href={`${params.value}`}
            target="_blank"
            className="break-all underline text-red-600"
            rel="noreferrer"
          >
            {name}
          </a>
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        switch (stats) {
          case "waiting":
            return (
              <div className=" rounded-full px-2 text-amber-600 border-2 border-amber-600">
                {params.value}
              </div>
            );
          case "approve":
            return (
              <div className=" rounded-full px-2 text-green-600 border-2 border-green-600">
                {params.value}
              </div>
            );
          case "decline":
            return (
              <div className=" rounded-full px-2 text-red-600 border-2 border-red-600">
                {params.value}
              </div>
            );
          default:
      ...
      renderCell: (params) => {
        return (
          <div className="flex flex-col gap-2 font-bold">
            <button
              className="bg-red-600 text-white py-[0.1rem] rounded px-2"
              onClick={() => handleStats(params.row.id, "approve")}
            >
              Approve
            </button>
            <button
              className="shadow text-red-600 py-[0.1rem] rounded px-2"
              onClick={() => handleStats(params.row.id, "decline")}
            >
              Decline
            </button>
          </div>
...
export default ListSubmissionUmkm;
